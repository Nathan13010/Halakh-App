...
async function main() {
  ...
  if (simanNum) {
    const found = allSimanim.find(s => s.siman === simanNum);
    if (found) simaninToProcess = [found];
    else {
...
const {simanNum} = {simanNum: 9}; let allSimanim = scanCompletDir(); console.log(allSimanim.filter(s=>s.siman===simanNum).map(s=>s.unique_key));