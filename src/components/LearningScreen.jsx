...
const LearningScreen = ({ xp = 0, onAddXp, streak = 1 }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-zinc-950 text-zinc-100 select-none overflow-hidden relative">
      {/* Top Gamification Status Bar */}
      <div className="bg-zinc-900/90 backdrop-blur-md border-b border-zinc-800/80 px-5 py-3 flex justify-between items-center shrink-0 z-20 shadow-md">
        <div className="flex items-center gap-3">
          {/* XP Badge */}
          <div className="flex items-center gap-1.5 text-amber-400 font-bold bg-amber-500/10 px-3.5 py-1.5 rounded-full border border-amber-500/30 text-xs shadow-inner">
            <span className="text-base">⚡</span>
            <span>{xp} XP</span>
          </div>

          {/* Streak Badge */}
          <div className="flex items-center gap-1.5 text-orange-400 font-bold bg-orange-500/10 px-3.5 py-1.5 rounded-full border border-orange-500/30 text-xs shadow-inner">
            <span className="text-base">🔥</span>
        ...
        <button
          ...
          className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-800/90 border border-zinc-700 hover:border-amber-500/50 text-zinc-200 text-xs font-bold transition-all cursor-pointer hover:scale-105 active:scale-95"
        >
      ...
      <div className="bg-gradient-to-r from-zinc-900 via-amber-950/20 to-zinc-900 border-b border-zinc-800/50 px-6 py-3 shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-amber-500 uppercase tracking-wider">Siman 1</span>
          <span className="text-xs font-semibold text-zinc-300">• Lois du Réveil Matinal</span>
        </div>
        <div className="flex items-center gap-3 w-1/3 max-w-[200px]">
          ...
          <span className="text-[11px] font-mono font-bold text-amber-400">{siman1ProgressPercent}%</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-10 relative flex flex-col items-center custom-scrollbar">
        ...
        <div className="absolute top-10 bottom-10 w-2.5 bg-gradient-to-b from-amber-500/40 via-blue-500/20 to-zinc-800 left-1/2 -translate-x-1/2 rounded-full pointer-events-none" />

        {/* Level Chapters & Nodes */}
        <div className="z-10 w-full max-w-md space-y-10 py-4">
          
          {/* Active Level 1: Siman 1 (Seifim 1 à 5) */}
          <div className="bg-zinc-900/90 border border-amber-500/30 rounded-3xl p-6 shadow-xl relative overflow-hidden backdrop-blur-sm">
            <div className="flex items-center justify-between mb-4 border-b border-zinc-800 pb-3">
              <div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20">
                  Niveau 1 • Siman 1 (Seifim 1-5)
                </span>
                <h3 className="text-lg font-serif font-bold text-zinc-100 mt-1.5">Les Fondements du Réveil</h3>
              </div>
                ...
                return (
                  <div key={quiz.lessonId} className={`flex flex-col items-center ${offsetStyles} group`}>
                    <div className="text-center mb-1.5">
                      <span className="text-[10px] font-bold text-zinc-400 bg-zinc-950 px-2.5 py-0.5 rounded-full border border-zinc-800 shadow-sm">
                      ...
                      className={`relative w-20 h-20 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-lg border-2 cursor-pointer
                        ${isCompleted 
                          ...
                          : isUnlocked 
                            ...
                            : "bg-zinc-900 border-zinc-800 text-zinc-600 shadow-none cursor-not-allowed"
                        }`}
                    >
                    ...
                    <span className="text-[11px] font-medium text-zinc-300 mt-2 max-w-[140px] text-center truncate">
          ...
          {LEARNING_LEVELS.slice(1).map((lvl) => (
            <div key={lvl.id} className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 opacity-60 relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xl text-zinc-500">
                    🔒
                  </div>
                  <div>
                    ...
                    <h4 className="text-base font-bold text-zinc-300">{lvl.title}</h4>
                  </div>
                </div>
      ...
      {activeQuizLesson && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-xl bg-zinc-900 border border-zinc-800 rounded-t-3xl sm:rounded-3xl flex flex-col shadow-2xl overflow-hidden max-h-[92vh] sm:max-h-[85vh]">
            ...
            <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950 shrink-0">
              <div className="flex items-center gap-3 flex-1 mr-4">
                <div className="flex-1 bg-zinc-800 h-3 rounded-full overflow-hidden border border-zinc-700">
                  <div 
                ...
                return (
                  <>
                    <div className="space-y-2">
                      <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-amber-500 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20">
                        {activeQuizLesson.title}
                      </span>

                      {q.imageUrl && (
                        <div className="rounded-2xl overflow-hidden border border-zinc-800/80 shadow-lg my-3 aspect-video bg-zinc-950">
                          <img 
                            src={q.imageUrl} 
                      ...
                      {q.type === 'match' && q.hebrewWord && (
                        <div className="bg-zinc-950 border border-amber-500/30 p-4 rounded-2xl text-center my-3">
                          <span className="text-2xl font-hebrew-serif font-bold text-amber-400">{q.hebrewWord}</span>
                        </div>
                      )}

                      <h3 className="text-base sm:text-lg font-serif font-semibold text-zinc-100 leading-relaxed pt-2">
                    ...
                    <div className="space-y-3 pt-2">
                      {q.options.map((optText, optIdx) => {
                        ...
                        let optionStyle = "border-zinc-800 bg-zinc-950/60 text-zinc-300 hover:border-zinc-700 hover:bg-zinc-800/50";
                        
                        if (isSelected) {
                          optionStyle = "border-blue-500 bg-blue-500/10 text-blue-200 ring-2 ring-blue-500/30";
                        }
                        if (isSubmitted && isSelected) {
                          optionStyle = isCorrect 
                            ? "border-emerald-500 bg-emerald-500/10 text-emerald-200 ring-2 ring-emerald-500/40"
                            : "border-red-500 bg-red-500/10 text-red-200 ring-2 ring-red-500/40";
                        }
                        ...
                        return (
                          ...
                          >
                            <span className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 border ${
                              isSelected 
                                ? (isSubmitted ? (isCorrect ? 'bg-emerald-500 border-emerald-400 text-zinc-950' : 'bg-red-500 border-red-400 text-zinc-950') : 'bg-blue-500 border-blue-400 text-white')
      ...
      {showVictoryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
          ...
          <div className="w-full max-w-sm bg-zinc-900 border border-amber-500/50 rounded-3xl p-6 text-center space-y-6 shadow-2xl relative z-10 animate-scale-up">
            <div className="text-5xl animate-bounce">🏆</div>

            <div>
              <span className="text-xs font-mono font-bold uppercase tracking-widest text-amber-400">Leçon Complétée !</span>
              <h2 className="text-2xl font-serif font-bold text-zinc-100 mt-1">Félicitations !</h2>
            </div>

            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-4 flex justify-around items-center">
              <div>
                <span className="text-[10px] text-zinc-500 font-bold block uppercase">Points Gagnés</span>
                <span className="text-xl font-black text-amber-400">+{victoryXpEarned} XP</span>
              </div>
      ...
      {showBadgesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-3xl p-6 space-y-6 shadow-2xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4 shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-2xl">🏆</span>
                <h3 className="text-lg font-serif font-bold text-zinc-100">Mes Trophées & Badges</h3>
              </div>
              <button
                onClick={() => setShowBadgesModal(false)}
                className="p-1.5 rounded-full bg-zinc-800 hover:bg-zinc-700 text-zinc-400"
              >
                <Icon name="close" className="w-5 h-5" />
              </button>
            </div>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border flex items-center gap-4 transition-all ${
                      isUnlocked 
                        ...
                        : "bg-zinc-950/60 border-zinc-800/80 opacity-50"
                    }`}
                  >
                    <div className="text-3xl shrink-0">{b.icon}</div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-bold text-zinc-100">{b.title}</h4>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          isUnlocked ? "bg-amber-500 text-zinc-950" : "bg-zinc-800 text-zinc-500"
                        }`}>
                          {isUnlocked ? "Débloqué" : `Verrouillé`}
                        </span>
                      </div>
                      <p className="text-xs text-zinc-400 mt-1 leading-snug">{b.description}</p>
                    </div>
                  </div>
                );
              })}
